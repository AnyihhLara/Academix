  ...
  const result = await sequelize
    .transaction(async (t) => {
      let result = await sequelize.query(
        `SELECT * FROM ${table} WHERE academic_situation_id = :academic_situation_id and disable=FALSE`,
        {
  ...
  const result = await sequelize
    .transaction(async (t) => {
      const result = await sequelize.query(
        `DELETE FROM ${table} WHERE academic_situation_id = :academic_situation_id`,
        {
          replacements: { academic_situation_id },
...
export async function PUT({ params, request }) {
  ...
  const result = await sequelize.transaction(async (t) => {
    ...
    return await sequelize.query(
      `SELECT * FROM ${table} WHERE academic_situation_id = :academic_situation_id`,
      {
        type: sequelize.QueryTypes.SELECT,
        transaction: t,
        replacements: { academic_situation_id },
